...
def make_dataset(file_name, min_data_size=2000, quantile=0.1, delay=0):
    print('min_data_size = ', min_data_size,
          'quantile = ', quantile,
          'delay = ', delay)
    data_preprocess(file_name, min_data_size, quantile)
    ...
    for file_name in data_files:
        ...
        tmp[:, 0] = 110
        tmp[:, 1] = -1
        tmp[:, 2] = tmp_signal['timestamp'].astype(int) + (delay * 0.001)
        tmp[:, 3] = 0
